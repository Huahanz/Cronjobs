...
from Reminder.Models import nasdaqstock
...
class StockJob:
    ...
    def run(self, webcrawler, stock_obj):
        if webcrawler:
            ...
            if result:
                if self.conditionmanager.is_int_larger_than(result,
                                                            stock_obj.max) or self.conditionmanager.is_int_lower_than(
    ...
    def run_list(self):
        tsla = nasdaqstock.NasdaqStock('tsla', 'qwidget-dollar', 130, 190)
        yahoo = nasdaqstock.NasdaqStock('yhoo', 'qwidget-dollar', 36.9, 45)
        bac = nasdaqstock.NasdaqStock('bac', 'qwidget-dollar', 15, 18)
    ...
    def get_stock_list(self):
        stockmodel = nasdaqstockmodel.NasdaqStockModel()
        all_list = stockmodel.get_all()
